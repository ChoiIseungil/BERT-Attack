...
def get_important_scores(words, h, tgt_model, orig_prob, orig_label, orig_probs, tokenizer, batch_size, max_length):
    ...
    all_segs = []
    for text in texts:
        inputs = tokenizer.encode_plus(text, h, add_special_tokens=True, max_length=max_length, )
...
def attack(feature, tgt_model, mlm_model, tokenizer, k, batch_size, max_length=512, cos_mat=None, w2i={}, i2w={}, use_bpe=1, threshold_pred_score=0.3):
    ...
    # original label
    inputs = tokenizer.encode_plus(feature.p, feature.h, add_special_tokens=True, max_length=max_length, )
    ...
    for top_index in list_of_index:
        ...
        for substitute_ in substitutes:
            ...
            inputs = tokenizer.encode_plus(temp_text, feature.h, add_special_tokens=True, max_length=max_length, )
...
def evaluate(features):
    ...
    for feat in features:
        if feat.success > 2:
            ...
            if do_use == 1:
                sim = float(use.semantic_sim([feat.p], [feat.final_adverse]))
                if sim < sim_thres:
                    continue
